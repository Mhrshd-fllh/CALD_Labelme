            ...
            for image in images:
                ...
                for output, aug_box, aug_image in zip(outputs, aug_boxes, aug_images):
                    ...
                    for ab, ref_score_cls, ref_pm, ref_score in zip(aug_box, ref_scores_cls, prob_max, ref_scores):
                        ...
                        iou[width < 0] = 0.0
                        iou[height < 0] = 0.0
                        max_class_index = torch.argmax(ref_score_cls)
                        class_prob_original = ref_score_cls[max_class_index].item()
                        q = scores_cls[torch.argmax(iou)].cpu().numpy()
                        class_prob_augmented=q[max_class_index].item()
                        x =  abs(class_prob_original - class_prob_augmented)
                        ...
                        consistency_img = min(consistency_img, torch.abs(
                            torch.max(iou) + 0.5 * (1 - x) * (ref_pm + pm[torch.argmax(iou)]) - args.bp).item())
                        mean_img.append(torch.abs(
                            torch.max(iou) + 0.5 * (1 - x) * (ref_pm + pm[torch.argmax(iou)])).item())
